...
{
std::cout<<"theTrack->GetDefinition()->GetPDGEncoding()"<<theTrack->GetDefinition()->GetPDGEncoding()<<std::endl;
 //Print_Hit_Info();
...
{
  LogDebug("CTPPS_Diamond_SD") << "CTPPS_Diamond_SD: Collection " << theHC->GetName() << std::endl;
  //theHC->PrintAllHits();
} 
